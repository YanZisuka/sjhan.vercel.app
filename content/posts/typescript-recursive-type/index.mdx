...
훌륭하다. 한 가지 문제점은, 위 스니펫에서 변수 theme는 `Record<K, V>`타입으로 표현 가능할테고, 이건 타입스크립트가 훌륭하게 추론해낼 수 있는 타입이다.
...
interface CssColorProperties<
  ...
  ThemeAwareValue = ThemeToken<'colors', ThemeType>,
> {
  color?:
...
좋다. `color: 't`까지만 타이핑하더라도 textPrimary라는 후보가 추론되기 시작했을 거다.
...
```ts title=sx.ts highlight=1,5
interface CssColorProperties<
  ThemeType extends Theme,
  ThemeAwareValue = ThemeToken<'colors', ThemeType>,
> extends CssPseudoSelectorProps,
    CssColorProperties<ThemeType, ThemeAwareValue> {
  color?:
    | Css.StandardProperties<number | string>['color']
    | ThemeAwareValue
    | undefined
} // Type 'CssColorProperties<ThemeType, ThemeAwareValue>' 형식은 자기 자신을 기본 형식으로 재귀적으로 참조합니다.ts(2310)
...
```ts title=sx.ts highlight=13,15
...
...
모두가 타입을 아주 잘 알고 있을 필요는 없지만, 이처럼 적절한 제약을 만든다면 조직의 생산성에 도움이 될 수 있다.
